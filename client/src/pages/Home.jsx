...
import { FaChevronLeft, FaChevronRight, FaHeart, FaUtensils, FaBoxOpen } from "react-icons/fa";
...
export default function Home() {
  ...
  const toggleFav = (id) => {
    ...
    if (!uid) {
      ...
      navigate("/login", { state: { redirectTo: "/Favourites" }, replace: true });
      return;
    }
...
<section className="relative w-full bg-[#fcf3e0] overflow-hidden">
  ...
  <div className="absolute top-0 w-full -translate-y-1">
    <svg viewBox="0 0 500 150" preserveAspectRatio="none" className="w-full h-32">
      <path d="M0,50 C150,150 350,-50 500,50 L500,0 L0,0 Z" fill="#8B4513" />
    </svg>
  </div>
  ...
  <div className="max-w-7xl mx-auto px-6 py-24 md:py-32 flex flex-col-reverse md:flex-row items-center gap-10 relative">
    ...
    <div className="flex flex-col md:w-1/2 space-y-6 z-20 text-center md:text-left">
      <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-gray-900 leading-tight">
        <span className="block text-yellow-600">Indulge</span> in the Coastal Sweetness
      </h1>
      <p className="text-gray-700 md:text-lg max-w-md mx-auto md:mx-0">
        Handcrafted Belgian waffles, topped with creamy chocolate, fresh fruits, and a sprinkle of seaside magic. Taste happiness in every bite.
      </p>
    ...
    <div className="relative md:w-1/2 w-full flex justify-center md:justify-end z-10">
      ...
      <div className="absolute -top-6 -left-6 text-yellow-400 text-3xl animate-bounce">🧇</div>
      <div className="absolute bottom-8 right-8 text-yellow-500 text-4xl animate-pulse">🧇</div>
      <div className="absolute top-1/2 -right-10 text-yellow-300 text-2xl animate-bounce">🧇</div>
...
<section className="max-w-7xl mx-auto px-12 py-12 bg-[#f0deb7] rounded-xl mt-10">
  ...
  {loading ? (
    ...
    <div className="overflow-x-hidden">
      <Slider items={featured} favs={favs} toggleFav={toggleFav} className="py-4" />
    </div>
  )}
        ...
        {items.map((p) => (
          ...
          >
            ...
            >
              <FaHeart
                size={20}
                className={favs.includes(p._id) ? "text-red-500" : "text-gray-600"}
              />
            </button>
          </div>
        ))}
