...
export default function AdminOrders() {
  ...
  return (
    <div className="p-6 min-h-screen bg-[#EFCC89]">
      <h1 className="text-3xl font-bold mb-6 text-center text-yellow-900">Orders</h1>
      <div className="overflow-x-auto">
        <table className="min-w-full border border-yellow-800 rounded-xl shadow-lg bg-[#fff8f0] border-collapse">
          ...
          <tbody>
            {orders.length > 0 ? (
              orders.map(order => (
                <tr key={order._id} className="hover:bg-[#fff0da] transition-all text-center">
                  <td className="py-3 px-4 border">{order._id}</td>
                  <td className="py-3 px-4 border">
                    <div className="font-medium">{order.user?.name || currentUser?.name}</div>
                    <div className="text-sm text-gray-600">{order.user?.email || currentUser?.email}</div>
                  </td>
...
<td className="py-3 px-4 border text-left align-top">
  <ul className="space-y-2">
    {order.orderItems?.map((item, i) => (
      <li key={i} className="flex items-center gap-2 flex-wrap max-w-[220px]">
        <img
